...
def build_continual_dataloader(args):
    dataloader = list()
    class_mask = list() if args.task_type=='til' else None

    transform_train = None # build_transform(True)
    transform_val = None # build_transform(False)

    mode = args.mode = args.task_type
    print("##############", mode)
        ...
        if 'iDigits' in args.dataset:
            ...
            for i, dataset in enumerate(dataset_list):
                ...
                for i in range(len(splited_dataset)):
                    # print(splited_dataset[i][0].dataset.classes)
                    # exit()
            ...
            for i in range(args.num_tasks):
                ...
                splited_dataset.append((train[i], val[i]))
    ...
    for i in range(len(splited_dataset)):
        ...
        dataloader.append({'train': dataset_train, 'test': dataset_val})
...
def get_dataset(dataset, transform_train, transform_val, args):
    ...
    elif 'CORe50' in dataset:
        dataset_train = CORe50(args.data_path, train=True, download=True, transform=transform_train, mode=args.mode).data
        dataset_val = CORe50(args.data_path, train=False, download=True, transform=transform_val, mode=args.mode).data

    elif 'DomainNet' in dataset:
        dataset_train = DomainNet(args.data_path, train=True, download=True, transform=transform_train, mode=args.mode).data
        dataset_val = DomainNet(args.data_path, train=False, download=True, transform=transform_val, mode=args.mode).data
