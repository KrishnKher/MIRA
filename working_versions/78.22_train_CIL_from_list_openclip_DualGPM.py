# python train_CIL_from_list_openclip_DualGPM_copy2.py --infer_after 2000 --dataset inetR --num_classes 40 --epochs 5 --later_epochs 15 --lr 1e-3 --batch_size 128
...
if args.base_model == 'laion':
    ...
    def make_hopfield(model, domain):
        ...
        all_keys = []
        for adapters in adapter_list[1:3]:
...
for domain_idx, domain in enumerate(train_domains):
    ...
    opt = optim.AdamW(model_hopfield_keys + train_module_params + list(classifier.parameters()), lr=args.lr, weight_decay=1e-4)
