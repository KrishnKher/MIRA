...
from torchvision.transforms import Compose, Resize, RandomCrop, RandomHorizontalFlip, Normalize, RandomResizedCrop, ToPILImage, ToTensor, CenterCrop, InterpolationMode
...
class DomainDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, preprocess, returns_domain=False):
    ...
    def __getitem__(self, idx):
        ...
        if is_hf_dataset:
            ...
            if isinstance(image, JpegImageFile) or isinstance(image, PngImageFile) or isinstance(image, Image.Image):
                # print("Here")
                item['image'] = self.to_tensor(image.convert("RGB"))
                item['image'] = self.preprocess(item['image'])
            else:
...
print("Saving weights to ", f"weights/train_domain_adapters_list_vit_{dataset_name}{args.name_tag}.pt")
torch.save(train_domain_adaters_list, f"weights/train_domain_adapters_list_vit_{dataset_name}{args.name_tag}.pt")
torch.save(classifiers, f"weights/train_domain_classifiers_vit_{dataset_name}{args.name_tag}.pt")