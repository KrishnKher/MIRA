...
class MultiheadAttention(Module):
    ...
    def __init__(
        ...
        self.num_prefixes = 16
        self.prefixes = nn.Parameter(torch.randn(1, self.num_prefixes, 768))
        nn.init.kaiming_uniform_(self.prefixes, a=math.sqrt(5))
    ...
    def forward(
        ...
        if self.use_hopfield:
            ...
            hopfield_keys = self.hopfield_keys / torch.norm(self.hopfield_keys, dim=-1, keepdim=True)
            ...
            prefixes = (weight_sim @ self.hopfield_values).reshape(query.shape[0], self.num_prefixes, 768) # b, value_dim
        else:
            prefixes = self.prefixes.repeat(query.shape[0], 1, 1) # b, num_prefixes, key_dim

        query = torch.cat([query, prefixes], dim=1) # b, L+num_prefixes, key_dim
        if self._qkv_same_embed_dim:
            q, k, v = F.linear(
        ...
        if self.batch_first and is_batched:
            attn_output = attn_output[:-self.num_prefixes, :, :]
