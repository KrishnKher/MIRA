...
parser.add_argument("--adapters_per_domain", type=int, default=2)
...
class CustomAttention(nn.Module):
    ...
    def add_hopfield_element(self, key=None):
        ...
        key = key.unsqueeze(-1) if key is not None else torch.zeros(768, 1).to('cuda:0')
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.use_hopfield:
            ...
            # print(hopfield_keys)
            # exit()
            self.weight_sim = sim_scores = (weight_query @ hopfield_keys) # b, num_memories
            self.weight_sim = attn_output_weights = self.separation_function(self.weight_sim, dim=-1) # b, num_memories
            if self.store_hopfield_similarities:
                self.sim_scores = sim_scores.detach()
                self.sep_scores = self.weight_sim.detach()
            # weight_sim.cuda()
            # print("Devices : ", weight_sim.device, self.hopfield_values.device)
            # self.hopfield_values = self.hopfield_values.to(weight_sim.device)
            # self.weights = weight_sim @ self.hopfield_values # b, value_dim
            # model.add_weighted_adapter(mem_names, self.weight_sim, f"hopfield_{domain_idx}", combination_type = 'cat')

            # model.set_adapter(f"hopfield_{domain_idx}")
            # assert model.active_adapters == [f"hopfield_{domain_idx}"]
            # adapter_list = torch.load('/data/ai22mtech12002/projects/WeightDG/weights/train_domain_adapters_list_laion_DomainNet-dil_peft_saksham_lora_custom_98.pt', weights_only = False)
...
@torch.no_grad()
def set_peft_lora_weights(model: nn.Module, weight_dict: dict, adapter_name: str = "default"):
        ...
        if i in weight_dict:
            ...
            for proj_name in ["q_proj", "v_proj"]: 
                try:
                    ...
                    lora_A_layer = proj_layer.lora_A[adapter_name]
                    lora_B_layer = proj_layer.lora_B[adapter_name]
...
if dataset_name == "PACS":
    ...
    # adapter_list = torch.load('/data/ai22mtech12002/projects/WeightDG/weights/train_domain_adapters_list_laion_DomainNet-dil_peft_saksham_lora_custom_98.pt', weights_only = False)
    adapter_list = torch.load('/data/ai22mtech12002/projects/WeightDG/weights/train_domain_adapters_list_laion_DomainNet-dilstage1_peft_new_2508.pt', weights_only=False)
    # print(adapter_list)
    # print(adapter_list[0].keys())
    # print(adapter_list[0][0].shape)
...
if args.base_model == 'laion':
    ...
    def make_hopfield_new(model, domain, mem_names):
        ...
        for m, mem_name in enumerate(mem_names):
            set_peft_lora_weights(model, adapter_list[m][domain], adapter_name=mem_name)
        for name, module in model.named_modules():
            if isinstance(module, CustomAttention):
    ...
    def make_hopfield(model, domain):
        ...
        for adapters in adapter_list:
            ...
            for name, module in model.named_modules():
                if isinstance(module, CustomAttention):
                    if not module.use_hopfield:
                        # print(f"Module {name} does not use hopfield, skipping.")
                        # hopfield_query_module = nn.Sequential(
                    ...
                    # print("KEYS" , keys)
                    set_peft_lora_weights(model, adapters)
        ...
        for name, module in model.named_modules():
            ...
            if isinstance(module, peft.tuners.lora.layer.Linear):
                ...
                #     if attr.startswith('hopfield'):
                #         print(f"Setting {name}.{attr} to True")
                    # print(f"{name}.{attr}: {getattr(module, attr)}" )
                module.use_hopfield = True
                # print(f"Set use_hopfield=True for {name}")
        # exit(0)
        return all_keys, train_module_params
...
# num_mem = len(adapter_list)
# mem_names = [f"mem_{i}" for i in range(num_mem)]

# for name in mem_names:
#     model.add_adapter(name, lora_cfg)
#     model.set_adapter(name)
#     assert name in model.active_adapters, f"Adapter {name} not added to model."
...
for domain_idx, domain in enumerate(train_domains):
    ...
    for key in keys:
        print(key.shape, key.requires_grad)
        break
    ...
    for key_net in key_nets:
        print(key_net.shape, key_net.requires_grad)
        break
    if dualGPM is None:
    ...
    print("Model keys:")
    for key in keys:
        print(key.shape, key.requires_grad, key.grad)
        break
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            hopfield_masks = 1
            labels = new_labels
            # model_weights = model.forward_intermediate(pixel_values)
