...
def eval(domain_idx):
    ...
    test_loader = test_domain_loaders[domain_idx]
    ...
    for step, batch in enumerate(test_loader):
        ...
        acc_eval = (outputs.argmax(dim=1) == labels).float().mean().item()
        pbar.update(step + 1, values=[("loss", loss_eval.item()), (f"test acc {domain_idx}", acc_eval)])
        
    domain_accs[domain_idx] = pbar.get_values()[f'test acc {domain_idx}']
...
print("Saving weights to ", f"weights/model_without_adapters_{args.base_model}_{dataset_name}{args.name_tag}.pt")

torch.save(model.state_dict(), f"weights/model_without_adapters_{args.base_model}_{dataset_name}{args.name_tag}.pt")
torch.save(classifier.state_dict(), f"weights/classifier_without_adapters_{args.base_model}_{dataset_name}{args.name_tag}.pt")
