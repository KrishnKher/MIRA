...
#         #     self.in_proj.register_parameter("in_proj_weight", None)
...
#         )
#         self.q_proj = get_peft_model(self.q_proj, config)
#         self.v_proj = get_peft_model(self.v_proj, config)
...
#             # print(self.separation_function)
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        merged_mask, _ = self.merge_masks(attn_mask, key_padding_mask, query)
        ...
        attn_output, attn_output_weights = F.multi_head_attention_forward(
            ...
            training=self.training,
            key_padding_mask=None,  # handled in merged_mask
            need_weights=need_weights,
    ...
    def merge_masks(
        ...
        if attn_mask is not None:
            ...
            mask_type = 2
            if attn_mask.dim() == 3:
                attn_mask_expanded = attn_mask.view(batch_size, -1, tgt_len, tgt_len)
            else:
                attn_mask_expanded = attn_mask.view(1, 1, tgt_len, tgt_len).expand(
                    batch_size, self.num_heads, -1, -1
                )
            merged_mask = attn_mask_expanded
