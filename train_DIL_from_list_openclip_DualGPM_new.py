...
parser.add_argument('--lr', type=float, default=3e-3) #3e-3
...
state_dict = torch.load(
    '/data/ai22mtech12002/projects/WeightDG/weights/train_domain_classifiers_laion_DomainNet-dilstage_1_with_hopfield_3008.pt',
    map_location="cuda",
    weights_only=False  
)
...
# classifier.load_state_dict(state_dict[0].state_dict())
# classifier = torch.load('/data/ai22mtech12002/projects/WeightDG/weights/train_domain_classifiers_laion_DomainNet-dilstage_1_with_hopfield_3008.pt', weights_only=False)
# classifier = nn.Linear(512, num_classes).cuda()
# torch.load_state_dict(classifier, torch.load('/data/ai22mtech12002/projects/WeightDG/weights/train_domain_classifiers_laion_DomainNet-dilstage_1_with_hopfield_3008.pt'))
...
def eval():
    ...
    for domain_idx, test_loader in enumerate(test_domain_loaders):
        ...
        # classifier.load_state_dict(state_dict[domain_idx].state_dict())
...
#     with open(f'{parent_dir}/{args.base_model}_{args.dataset}_{args.name_tag}_2.pkl', 'wb') as f:
#         pkl.dump(model, f)
...
#             prefix = f'resblocks.{module_index}.attn.' + ('q_proj' if 'q_proj' in name else 'v_proj')
#             key_list = keys_q if 'q_proj' in name else keys_v
#             value_list = values_q if 'q_proj' in name else values_v
...
#     # exit()
#     opt = optim.AdamW(keys_new  + list(classifier.parameters()), lr=args.lr, weight_decay=1e-2)
...
# with open(f'{parent_dir}/{args.base_model}_{args.dataset}_{args.name_tag}_2.pkl', 'wb') as f:
#     pkl.dump(model, f)
# with open(f'{parent_dir}/{args.base_model}_{args.dataset}_{args.name_tag}_classifier_2.pkl', 'wb') as f:
...
# model = pkl.load(open(f'{parent_dir}/{args.base_model}_{args.dataset}_{args.name_tag}_2.pkl', 'rb'))
# classifier = pkl.load(open(f'{parent_dir}/{args.base_model}_{args.dataset}_{args.name_tag}_classifier_2.pkl', 'rb'))


classifier = pkl.load(open('/data/ai22mtech12002/projects/WeightDG/data/DomainNet-dil/laion_DomainNet-dil__classifier.pkl', 'rb'))
model_init = pkl.load(open('/data/ai22mtech12002/projects/WeightDG/data/DomainNet-dil/laion_DomainNet-dil_.pkl', 'rb'))
# model_init = torch.load(
#     '/data/ai22mtech12002/projects/WeightDG/weights/train_domain_adapters_list_laion_DomainNet-dilstage_1_with_hopfield_3008.pt',
#     map_location="cuda",
#     weights_only=False  
# )
...
# exit()
        module.use_hopfield = True
        search_key = name + '.hopfield_keys'
        search_value = name + '.hopfield_values'
        
        # module.hopfield_keys = hopfield_keys
        
        for n, m in model_init.named_modules():
            if n == name:
                # print(m.hopfield_keys.shape, m.hopfield_values.shape)
                module.hopfield_keys = nn.Parameter(m.hopfield_keys.cuda().clone().detach())
                module.hopfield_values = m.hopfield_values.cuda()
                # print(module.hopfield_keys)
                hopfield_keys[name] = m.hopfield_keys
                hopfield_values[name] = module.hopfield_values

# print(hopfield_keys.keys())

#         module.hopfield_keys = nn.Parameter(model_init.get_submodule(search_key).cuda().clone().detach())
#         module.hopfield_values = model_init.get_submodule(search_value).cuda()
#         module.hopfield_keys = nn.Parameter(model_init.state_dict()[f'{name}.hopfield_keys'].cuda().clone().detach())
#         module.hopfield_values = model_init.state_dict()[f'{name}.hopfield_values'].cuda()
# for domain_idx, domain in enumerate(train_domains):
#     print(f"Domain {domain}, idx {domain_idx}")
#     domain_adapters[domain_idx] = {
#         "keys_q" : [
#             hopfield_keys[f'base_model.model.transformer.resblocks.{i}.attn.q_proj'][:, domain_idx :: args.num_tasks]
#             for i in range(12)
#         ],
#         "values_q" : [
#             hopfield_values[f'base_model.model.transformer.resblocks.{i}.attn.q_proj'][domain_idx :: args.num_tasks, :]
#             for i in range(12)
#         ],
#         "keys_v" : [
#             hopfield_keys[f'base_model.model.transformer.resblocks.{i}.attn.v_proj'][:, domain_idx :: args.num_tasks]
#             for i in range(12)
#         ],
#         "values_v" : [
#             hopfield_values[f'base_model.model.transformer.resblocks.{i}.attn.v_proj'][domain_idx :: args.num_tasks, :]
#             for i in range(12)
#         ]
#     }
    
# # print(domain_adapters[0]['keys_q'][0] == domain_adapters[1]['keys_q'][0])
# for k, v in domain_adapters.items():
#     print(f"Domain {k}")
#     # for i in range(12):
#         # print(f"Layer {i}: keys_q {v['keys_q'][i].shape}, values_q {v['values_q'][i].shape}, keys_v {v['keys_v'][i].shape}, values_v {v['values_v'][i].shape}")

# with open("domain_wise_hopfield_keys.txt", "w") as f:
#     f.write(str(domain_adapters))
eval_accs = eval()
